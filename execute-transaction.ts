...
export const executeTransaction: Handler<TransactionRequest, string> = async (request: TransactionRequest) => {
    console.log('execute transaction invoked')
    ...
    independentProducts.forEach(async (product) => {
        console.log(`executing ${product.product}`)
    ...
    if (dependentProducts && dependentProducts.length > 0) {
        await poll(POLL_RATE, POLL_TIMES, async (interval, resolve) => {
